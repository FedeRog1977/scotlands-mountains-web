...
\pagenumbering{gobble}
	...
	\author{\textsc{Lewis W. Britton, L.W.B.}\\
                \textsc{202194412}\\
...
\section{System Design}\label{ch5}
	...
	All data discussed in Section 4 of this report is included in a basic input-output system [(BIOS, wheyy)] which, through functions and others methods, produces the desired outcomes which satisfy user requirements. A summary of this process can be seen below in Table 5.2. Reference to data can be observed in Figure A4.
	...
	\begin{tabular}{p{2.5cm}p{2.5cm}p{2.5cm}p{2.5cm}p{2.5cm}}
		\hline
		\hline
		\textsc{Function} & \textsc{Inputs} & \textsc{Data} & \textsc{Return Type} & \textsc{Output}\\
		\hline
		\hline
		\multicolumn{5}{c}{\textsc{`Overview'}}\\
		\hline
		\multicolumn{5}{c}{\textsc{`Conditioning'}}\\
		\hline
		\multicolumn{5}{c}{\textsc{`Weather'}}\\
		\hline
		\multicolumn{5}{c}{\textsc{`Conquest Map'}}\\
		\hline
		\hline
	\end{tabular}
		\caption{Data Input-Output Summary}
	\end{center}
	\end{table}
	...
	Henceforth, scoring a route takes into account [1] four groups of subjective scores based on weights and values of the four route element factors seen under routes in the JSON file (type, stages, terrain type(s), terrain difficulties), and [2] a more objective and logical multiplier based on total number of [Munro, `Top, Corbett, `Top] tops covered on-route, the total elevation gained, and the total distance covered. Actual values of the weights were gathered through personal consideration and conversation in the field and is therefore subject to constant adjustment. So forth, the multiplier was developed purely by myself. All parameters of this equation are as follows:
	...
	\setlength\itemsep{0cm}
		...
		\item Terrain type(s) (weight, \{weights, `values'\}, `score') -- matrix
...
\newpage
	...
	[1.1] \textbf{Conversion Constants}:
	...
	$N_{i,j,k,l}=$ Numerical value representing the position of a string element $i,j,k,l$ in arrays of $i$: route type, $j$: route stages, $k$: terrain type(s), $l$: terrain difficulties; $i\in\{1,...I\}$, $j\in\{1,...J\}$, $k\in\{1,...K\}$, $l\in\{1,...L\}$\\

	[1.3] \textbf{Elevation}:

	$$\mathrm{Elev_{m}}=c_{ft}\mathrm{Elev_{ft}}$$

	Where:\\
	$\mathrm{Elev_{m}=}$ Total elevation gained on given route, in meters (m)\\
	$\mathrm{Elev_{ft}=}$ Total elevation gained on given route, in feet (ft)\\

	[1.4] \textbf{Distance}:

	$$\mathrm{Dist_{km}}=c_{mi}\mathrm{Dist_{mi}}$$

	Where:\\
	$\mathrm{Dist_{km}=}$ Total distance covered on given route, in kilometers (km)\\
	$\mathrm{Dist_{mi}=}$ Total distance covered on given route, in miles (mi)\\

\newpage

	[2.1] \textbf{Weights} for Route Type, Route Stages, Terrain Type(s), Terrain Difficulties Elements:\\

	This method defines equally weighted elements by dividing 1 by the number of elements in the arrays.
	...
	Where:\\
	$w_{i,j,k,l}=$ Equally weighted value based on number of elements for element $i,j,k,l$ in arrays of $i$: route type, $j$: route stages, $k$: terrain type(s), $l$: terrain difficulties; $i\in\{1,...I\}$, $j\in\{1,...J\}$, $k\in\{1,...K\}$, $l\in\{1,...L\}$\\
	$N_{i,j,k,l}=$ Numerical value representing the position of a string element $i,j,k,l$ in arrays of $i$: route type, $j$: route stages, $k$: terrain type(s), $l$: terrain difficulties; $i\in\{1,...I\}$, $j\in\{1,...J\}$, $k\in\{1,...K\}$, $l\in\{1,...L\}$\\

	[2.2] \textbf{Scores} for Route Type, Route Stages, Terrain Type(s), Terrain Difficulties:

	$$S_{i}=\sum^{I}_{i=1} w_{i}V_{i},\ S_{j}=\sum^{J}_{j=1} w_{j}V_{j},\ S_{k}=\sum^{K}_{k=1} w_{k}V_{k},\ S_{l}=\sum^{L}_{l=1} w_{l}V_{l},$$

	Where:\\
	$S_{i,j,k,l}=$ Relative score of element $i,j,k,l$ in arrays of $i$: route type, $j$: route stages, $k$: terrain type(s), $l$: terrain difficulties; $i\in\{1,...I\}$, $j\in\{1,...J\}$, $k\in\{1,...K\}$, $l\in\{1,...L\}$\\
	$V_{i,j,k,l}=$ Subjectively defined value assigned to element $i,j,k,l$ in arrays of $i$: route type, $j$: route stages, $k$: terrain type(s), $l$: terrain difficulties; $i\in\{1,...I\}$, $j\in\{1,...J\}$, $k\in\{1,...K\}$, $l\in\{1,...L\}$\\
...
\newpage
	...
	[2.3] \textbf{Weights} for Route Type, Route Stages, Terrain Type(s), Terrain Difficulties Elements:
	...
	Where:\\
	$w_{S_{i,j,k,l}}=$ Weight values determining the importance of arrays of $i$: route type, $j$: route stages, $k$: terrain type(s), $l$: terrain difficulties; $i\in\{1,...I\}$, $j\in\{1,...J\}$, $k\in\{1,...K\}$, $l\in\{1,...L\}$\\

	[3.1] \textbf{Complete Multiplier}:

	$$m_{R}=\frac{N_{\mathrm{Tops}}}{\left(\frac{\mathrm{Elev_{m}}}{1000\left(\mathrm{Dist_{km}}\right)}\right)}$$
	$$\textrm{Expanded: }m_{R}=\frac{\sum^{I}_{i=1}N_{i}+\sum^{J}_{j=1}N_{j}+\sum^{K}_{k=1}N_{k}+\sum^{L}_{l=1}N_{l}}{\left(\frac{c_{ft}\mathrm{Elev_{ft}}}{1000\left(c_{mi}\mathrm{Dist_{mi}}\right)}\right)}$$
	...
	[3.2] \textbf{Complete Aggregate Route Score}:

	$$S_{R}=w_{S_{i}}S_{i}+w_{S_{j}}S_{j}+w_{S_{k}}S_{k}+w_{S_{l}}S_{l}$$
	$$\textrm{Expanded (1): }S_{R}=w_{S_{i}}\left(\sum^{I}_{i=1} w_{i}V_{i}\right)+...+w_{S_{l}}\left(\sum^{L}_{l=1} w_{l}V_{l}\right)$$
	$$\textrm{Expanded (2): }S_{R}=w_{S_{i}}\left(\sum^{I}_{i=1} \left(\frac{1}{\sum^{I}_{i=1}N_{i}}\right)V_{i}\right)+...+w_{S_{l}}\left(\sum^{L}_{l=1} \left(\frac{1}{\sum^{L}_{l=1}N_{l}}\right)V_{l}\right)$$

	Where:\\
	$S_{R}=$ Aggregate route score for given route $R$\\

	[4] \textbf{Complete Adjusted Route Score}:

	$$S_{R(A)}=m_{R}\left(w_{S_{i}}S_{i}+w_{S_{j}}S_{j}+w_{S_{k}}S_{k}+w_{S_{l}}S_{l}\right)$$
	$$\textrm{Expanded (1): }S_{R(A)}=\left(\frac{N_{\mathrm{Tops}}}{\left(\frac{\mathrm{Elev_{m}}}{1000\left(\mathrm{Dist_{km}}\right)}\right)}\right)\left(w_{S_{i}}\left(\sum^{I}_{i=1} w_{i}V_{i}\right)+...+w_{S_{l}}\left(\sum^{L}_{l=1} w_{l}V_{l}\right)\right)$$
	$$\textrm{Expanded (2): }S_{R(A)}=\left(\frac{\sum^{I}_{i=1}N_{i}+\sum^{J}_{j=1}N_{j}+\sum^{K}_{k=1}N_{k}+\sum^{L}_{l=1}N_{l}}{\left(\frac{c_{ft}\mathrm{Elev_{ft}}}{1000\left(c_{mi}\mathrm{Dist_{mi}}\right)}\right)}\right)$$
	$$+\left(w_{S_{i}}\left(\sum^{I}_{i=1} \left(\frac{1}{\sum^{I}_{i=1}N_{i}}\right)V_{i}\right)+...+w_{S_{l}}\left(\sum^{L}_{l=1} \left(\frac{1}{\sum^{L}_{l=1}N_{l}}\right)V_{l}\right)\right)$$

	Where:\\
	$S_{R(A)}=$ Adjusted route score for given route $R$\\

	In English, this equation reads: ``number of tops (Munro, Munro Top, Corbett, Corbett Top) achieved on the route per vertical meter gained on the route, per horizontal meter gained on the route; scaled by the route type, route stages, terrain type(s), and terrain difficulty elements found on-route''.\\
	...
	The `Conditioning' portion of the home page of this site presents the user with the ability to [1] view `Ability' attribute, and [2] view `Equipment' attributes. This, like the `Overview' section is a simplified and more informative version of what the user will see and use ability and equipment data for in the `Conquest Map'.
			...
			\paragraph{Search `Ability'}

	Users are presented with an HTML \texttt{select} protocol on a two-tier basis. The first allows them to choose an \texttt{option} based on which set of ability factors they wish to inspect, such as hill and route elements, summit features, route types and stages, terrain types and difficulties, etc. This generates the second \texttt{select} protocol which displays the individual factors under each heading. When the user selects a factor, the element is matched by a loop through the relevant JSON file for the seven ability headings and a body of text is then generated summarizing the attributes. This includes all of the basics such as a brief description and images, etc. This feature is informative and serves no dynamic function until `Conquest Map'.
	
			\paragraph{Search `Equipment'}

	Users are presented with an HTML \texttt{select} protocol, again on a two-tier basis. The first allows them to choose an \texttt{option} based on which set of equipment factors they wish to inspect, such as packs, technical equipment, footwear and clothing, etc. This generates the second \texttt{select} protocol which displays the individual factors under each heading. Upon selection of a factor, the element is matched by a loop through the relevant JSON file for the four equipment headings and a body of text is then generated. This includes details such as a brief description and images, etc. Again, this feature is informative and serves no dynamic function until `Conquest Map'.

		\subsubsection{`Weather'}

	Fetching and putting weather data to use only requires two parameters: latitude and longitude (\texttt{lat}, \texttt{lon}). This leaves huge opportunity to relate these parameters to many different variables and therefore, many different human-interpreted scenarios.
...
\section{System Construction \& Implementation}\label{ch6}
	...
	\subsection{Development \& Languages}
		...
		\subsubsection{Supporting Data (JSON)}
