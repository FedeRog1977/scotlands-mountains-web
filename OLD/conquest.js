...
function createHillMarker(
  ...
  var popup =
    ...
    "</div>";
  var hillMarker = new L.marker([lat, lon], { icon: iconType })
    .addTo(map)
    .bindPopup(popup);
...
function createRouteMarker(
  ...
  var popup =
    "<h2 style='margin:0 0.5em 0.25em 0.5em;'>" +
    route +
    " - Start</h2>" +
    "<b>Distance: </b>" +
    dist +
    ...
    corbetts +
    corbetttops;
  var routeMarker = new L.marker(coords, { icon: iconType })
    .addTo(map)
    .bindPopup(popup);
...
function findLocation(e) {
  ...
  var currLocMarker = new L.marker(e.latlng, { icon: locationIcon })
    .addTo(map)
    .bindPopup("You&rsquo;re within an accuracy of " + radius + "m from here")
    .openPopup();
        ...
        for (var k in hillType) {
          ...
          if (distMi === minDist) {
            ...
            nearestOut.innerHTML =
              ...
              hillRoutes.join("<br>") +
              "</p><hr>" +
              "<p><a href='https://www.google.com/maps/search/current+location+to+" +
              hillName +
              "/'>Directions (Soyggle Maps)</a></p>";
          }
        }
      }
    });
...
function searchLocation() {
  fetch(locations)
    ...
    .then((data) => {
      const hills = data;
      const inpLocation = document
        .getElementById("inpLocation")
        .value.toLowerCase();
      ...
      for (var i in hills.landmass) {
        for (var k in hills.landmass[i].munro) {
          if (
            hills.landmass[i].munro[k].name.toLowerCase().match(inpLocation)
          ) {
            ...
            if (hills.landmass[i].subsubtype == null) {
              ...
              landmassSubsubtype =
                "<b>Prominent feature</b>: " +
                hills.landmass[i].subsubtype +
                "<br>";
            }
        ...
        for (var k in hills.landmass[i].munrotop) {
          if (
            hills.landmass[i].munrotop[k].name.toLowerCase().match(inpLocation)
          ) {
            landmassName = hills.landmass[i].name;
            landmassType = hills.landmass[i].type;
        ...
        for (var k in hills.landmass[i].corbett) {
          if (
            hills.landmass[i].corbett[k].name.toLowerCase().match(inpLocation)
          ) {
            landmassName = hills.landmass[i].name;
            landmassType = hills.landmass[i].type;
        ...
        for (var k in hillType) {
          if (hillName.toLowerCase().match(inpLocation)) {
            ...
            let hillFromTo = getDistanceCurr(hillBuff.lat, hillBuff.lon);
            ...
            locationOut.innerHTML =
              ...
              hillSum +
              "<br><hr>" +
              "<div style='text-align:center;'><img src='./Photos/" +
              hillImg +
              "' class='imghill'></img></div>";
...
function scoreRoute(elev, dist, nTops, type, stage, terrType, terrDiff) {
  ...
  if (type.includes("walk")) {
    ...
    typeValues = [2];
    typeScore = typeElementWeight * typeValues[0];
  } else if (type.includes("hillwalk") && type.includes("ridgewalk")) {
    typeValues = [0.5, 0.75];
    typeScore =
      typeElementWeight * typeValues[0] + typeElementWeight * typeValues[1];
  } else if (type.includes("hillwalk") && type.includes("scramble")) {
    typeValues = [0.5, 1.5];
    typeScore =
      typeElementWeight * typeValues[0] + typeElementWeight * typeValues[1];
  } else if (
    type.includes("hillwalk") &&
    type.includes("scramble") &&
    type.includes("climb")
  ) {
    typeValues = [0.5, 1.5, 2];
    typeScore =
      typeElementWeight * typeValues[0] +
  ...
  if (stage.includes("walk-in")) {
    ...
    stage.includes("walk-in") &&
    stage.includes("on-hill") &&
    stage.includes("walk-out")
  ) {
    stageValues = [1, 1, 1];
    stageScore =
      stageElementWeight * stageValues[0] +
  ...
  } else if (stage.includes("approach") && stage.includes("on-hill")) {
    stageValues = [1, 1];
    stageScore =
      stageElementWeight * stageValues[0] + stageElementWeight * stageValues[1];
  } else if (
    stage.includes("approach") &&
    stage.includes("on-hill") &&
    stage.includes("walk-out")
  ...
  if (terrType.includes("road") && terrType.includes("path")) {
    terrTypeValues = [0.75, 1];
    terrTypeScore =
      terrTypeElementWeight * terrTypeValues[0] +
      terrTypeElementWeight * terrTypeValues[1];
  } else if (
    terrType.includes("road") &&
  ...
  if (
    ...
    terrDiffScore =
      terrDiffElementWeight * terrDiffValues[0] +
...
function searchRoute(landmass) {
  fetch(locations)
    ...
    .then((data) => {
      const hills = data;
      const selectRoute = document
        .getElementById("selectRoute" + landmass)
        .value.toLowerCase();
      ...
      for (var i in hills.landmass) {
        for (var k in hills.landmass[i].route) {
          if (hills.landmass[i].route[k].name.toLowerCase() === selectRoute) {
            ...
            let routeElev =
              hills.landmass[i].route[k].elevationgain.toLocaleString("en-US");
            ...
            let routeTerrainType =
              hills.landmass[i].route[k].terraintype.join(", ");
            let routeTerrainDiff =
              hills.landmass[i].route[k].terraindiff.join(", ");
            ...
            if (hills.landmass[i].route[k].gear.length === 0) {
              ...
              routeGear =
                "<b>Gear:</b> " +
                hills.landmass[i].route[k].gear.join(", ") +
                "<br>";
            }
            ...
            if (hills.landmass[i].route[k].munro.length === 0) {
              ...
              routeMunros =
                "<b>Munro:</b> " + hills.landmass[i].route[k].munro + "<br>";
            } else {
              routeMunros =
                "<b>Munros:</b><br>" +
                hills.landmass[i].route[k].munro.join("<br>") +
                "<br>";
            }
            ...
            if (hills.landmass[i].route[k].munrotop.length === 0) {
              ...
              routeMunroTops =
                "<b>Munro Top:</b> " +
                hills.landmass[i].route[k].munrotop +
                "<br>";
            } else {
              routeMunroTops =
                "<b>Munro Tops:</b><br>" +
                hills.landmass[i].route[k].munrotop.join("<br>") +
                "<br>";
            }
            ...
            if (hills.landmass[i].route[k].corbett.length === 0) {
              ...
              routeCorbetts =
                "<b>Corbett:</b> " +
                hills.landmass[i].route[k].corbett +
                "<br>";
            } else {
              routeCorbetts =
                "<b>Corbetts:</b><br>" +
                hills.landmass[i].route[k].corbett.join("<br>") +
                "<br>";
            }
            ...
            if (hills.landmass[i].route[k].corbetttop.length === 0) {
              ...
              routeCorbettTops =
                "<b>Corbett Top:</b> " +
                hills.landmass[i].route[k].corbetttop +
                "<br>";
            } else {
              routeCorbettTops =
                "<b>Corbett Tops:</b><br>" +
                hills.landmass[i].route[k].corbetttop.join("<br>") +
                "<br>";
            }
  ...
  fetch(locations)
    ...
    .then((data) => {
      ...
      const selectRoute = document
        ...
        .value.toLowerCase();
      let loadRoutePrefix =
        "https://raw.githubusercontent.com/FedeRog1977/Burning/master/System/GPX/";
      let loadRouteSuffix = "";
      let loadRoute = "";
      let routeName = "";
      let routeDist = "";
      ...
      fetch(loadRoute)
        ...
        .then((doc) => {
          ...
          const route = data;
          var routeTrack = new L.geoJSON(route, { color: "#A80606" }).addTo(
            map
          );
...
function selectAbilityRefine(component) {
  const selectAbility = document
    .getElementById("selectAbility" + component)
    .value.toLowerCase();
  const selectAbilityRoutes = document.getElementById("selectAbilityRoutes");
  const selectAbilityRoutesTotalDistanceCont = document.getElementById(
    "selectAbilityRoutesTotalDistanceCont"
  );
  const selectAbilityRoutesTotalElevationCont = document.getElementById(
    "selectAbilityRoutesTotalElevationCont"
  );
  const selectAbilityRoutesTotalTimeCont = document.getElementById(
    "selectAbilityRoutesTotalTimeCont"
  );
  var selectAbilityRoutesTotalDistanceMin = document.getElementById(
    "selectAbilityRoutesTotalDistanceMin"
  );
  var selectAbilityRoutesTotalDistanceMax = document.getElementById(
    "selectAbilityRoutesTotalDistanceMax"
  );
  var selectAbilityRoutesTotalDistanceMinOut = document.getElementById(
    "selectAbilityRoutesTotalDistanceMinOut"
  );
  var selectAbilityRoutesTotalDistanceMaxOut = document.getElementById(
    "selectAbilityRoutesTotalDistanceMaxOut"
  );
  var selectAbilityRoutesTotalElevationMin = document.getElementById(
    "selectAbilityRoutesTotalElevationMin"
  );
  var selectAbilityRoutesTotalElevationMax = document.getElementById(
    "selectAbilityRoutesTotalElevationMax"
  );
  var selectAbilityRoutesTotalElevationMinOut = document.getElementById(
    "selectAbilityRoutesTotalElevationMinOut"
  );
  var selectAbilityRoutesTotalElevationMaxOut = document.getElementById(
    "selectAbilityRoutesTotalElevationMaxOut"
  );
  var selectAbilityRoutesTotalTimeMin = document.getElementById(
    "selectAbilityRoutesTotalTimeMin"
  );
  var selectAbilityRoutesTotalTimeMax = document.getElementById(
    "selectAbilityRoutesTotalTimeMax"
  );
  var selectAbilityRoutesTotalTimeMinOut = document.getElementById(
    "selectAbilityRoutesTotalTimeMinOut"
  );
  var selectAbilityRoutesTotalTimeMaxOut = document.getElementById(
    "selectAbilityRoutesTotalTimeMaxOut"
  );
  ...
  if (selectAbilityRoutes.value === "Total Distance") {
    ...
    selectAbilityRoutesTotalDistanceMinOut.innerHTML =
      selectAbilityRoutesTotalDistanceMin.value;
    selectAbilityRoutesTotalDistanceMaxOut.innerHTML =
      selectAbilityRoutesTotalDistanceMax.value;
    ...
    selectAbilityRoutesTotalTimeCont.classList.add("hidden");
    selectAbilityRoutesTotalElevationMinOut.innerHTML =
      selectAbilityRoutesTotalElevationMin.value;
    selectAbilityRoutesTotalElevationMaxOut.innerHTML =
      selectAbilityRoutesTotalElevationMax.value;
    ...
    selectAbilityRoutesTotalTimeMinOut.innerHTML =
      selectAbilityRoutesTotalTimeMin.value;
    selectAbilityRoutesTotalTimeMaxOut.innerHTML =
      selectAbilityRoutesTotalTimeMax.value;
  ...
  fetch(attributes)
    ...
    .then((data) => {
      ...
      const selectAbility = document
        .getElementById("selectAbility" + component)
        .value.toLowerCase();
      const refinementsOutShell = document.getElementById(
        "refinementsOutShell"
      );
      ...
      for (var i in attributes.elementsroute) {
        if (attributes.elementsroute[i].name.toLowerCase() === selectAbility) {
          ...
          abilityDistanceMin = document.getElementById(
            "selectAbilityRoutesTotalDistanceMin"
          ).value;
          abilityDistanceMax = document.getElementById(
            "selectAbilityRoutesTotalDistanceMax"
          ).value;
          abilityElevationMin = document.getElementById(
            "selectAbilityRoutesTotalElevationMin"
          ).value;
          abilityElevationMax = document.getElementById(
            "selectAbilityRoutesTotalElevationMax"
          ).value;
          abilityTimeMin = document.getElementById(
            "selectAbilityRoutesTotalTimeMin"
          ).value;
          abilityTimeMax = document.getElementById(
            "selectAbilityRoutesTotalTimeMax"
          ).value;
...
function addEquipmentRefine(component) {
  fetch(attributes)
    ...
    .then((data) => {
      const attributes = data;
      const selectEquipment = document
        .getElementById("selectEquipment" + component)
        .value.toLowerCase();
      const refinementsOutShell = document.getElementById(
        "refinementsOutShell"
      );
...
function searchRouteRefined(landmass) {
  fetch(locations)
    ...
    .then((data) => {
      const hills = data;
      const refinementsRoutesOut = document.getElementById(
        "refinementsRoutesOut"
      );
      refinementsRoutesOut.innerHTML = "";
      let loadRoutePrefix =
        "https://raw.githubusercontent.com/FedeRog1977/Burning/master/System/GPX/";
      let loadRouteSuffix = "";
      let loadRoute = "";
      ...
      for (var i in hills.landmass) {
        for (var k in hills.landmass[i].route) {
          ...
          if (
            ...
            hills.landmass[i].route[k].elevationgain >=
              refinementsVals.elevationMin &&
            hills.landmass[i].route[k].elevationgain <=
              refinementsVals.elevationMax
          ) {
